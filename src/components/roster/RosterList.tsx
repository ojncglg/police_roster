...
import type { SpecialTeam, Officer } from '../../types/officer';
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <span className="font-medium text-amber-600">
                      {selectedRoster.officers.filter(o => o.status === 'leave').length}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-600">In Training: </span>
                    <span className="font-medium text-blue-600">
                      {selectedRoster.officers.filter(o => o.status === 'training').length}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-600">Inactive: </span>
                    <span className="font-medium text-red-600">
                      {selectedRoster.officers.filter(o => o.status === 'inactive').length}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-600">Active: </span>
                    <span className="font-medium text-green-600">
                      {selectedRoster.officers.filter(o => o.status === 'active').length}
                    </span>
                  </div>
                </div>
              </div>
                  ...
                  {selectedRoster.officers.map((officer) => (
                    <div key={officer.id} className="bg-gray-50 p-2 rounded">
                      ...
                      {isSpecialUnit(officer.specialAssignment) && (
                        <span className="ml-2 px-2 py-0.5 bg-blue-100 text-blue-800 rounded-full font-bold text-sm">
                          {getUnitDisplayName(officer.specialAssignment)}
                        </span>
                      )}
              ...
              <div className="mt-4 bg-gray-100 p-4 rounded-lg">
                ...
                <div className="space-y-2">
                  {selectedRoster.officers
                    ...
                    .map((officer, index) => (
                      <div key={index} className="flex items-center space-x-2">
                        ...
                        <span className="px-2 py-0.5 bg-blue-100 text-blue-800 rounded-full font-bold text-sm">
                          {getUnitDisplayName(officer.specialAssignment)}
                        </span>
                  ...
                  {!selectedRoster.officers.some(o => isSpecialUnit(o.specialAssignment)) && (
                    <p className="text-gray-600">No officers with special units assigned</p>
                  )}
                </div>
          ...
          <div className="flex justify-between items-start">
            ...
            <div className="flex space-x-2">
              <button
                ...
                aria-label="View roster details"
              >
